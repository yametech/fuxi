...
func (t *Tracker) Monitor(allowed []string) <-chan []Run {
...
func (t *Tracker) findNewTaskruns(pr *v1alpha1.PipelineRun, allowed []string) []Run {
	var ret []Run
	for tr, trs := range pr.Status.TaskRuns {
		run := Run{Name: tr, Task: trs.PipelineTaskName}

		if t.loggingInProgress(tr) ||
			!HasScheduled(trs) ||
			IsFiltered(run, allowed) {
			continue
		}
