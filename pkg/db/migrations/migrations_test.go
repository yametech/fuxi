...
func TestMigration(t *testing.T) {
	db, err := gorm.Open("mysql", "root:000000@(10.200.100.219:4406)/test?charset=utf8&parseTime=True&loc=Local")
	...
	m := New(db, DefaultOptions, []*Migration{
		...
		{
			ID:          "201608301400",
			Description: "create persons table",
			Migrate: func(tx *gorm.DB) error {
		...
		{
			ID:          "201608301415",
			Description: "add age column to person table",
			Migrate: func(tx *gorm.DB) error {
